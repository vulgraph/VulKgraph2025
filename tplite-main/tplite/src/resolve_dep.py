...
def resolve_source_relation(tpl_id): # centris方法
    '''Identify reused functions'''
    # 检测tpl_id重用的函数
    res = dict() # {tpl_id:{tpl_id_x:func_hash}}    tpl_id重用源自tpl_id_x的func
    tpl_intersection = defaultdict(set) # tpl_intersection={tpl_id_x1:(func_hash1,func_hash2),tpl_id_x2:(func_hash3,func_hash4)}
    tpl_sig_s = tpl_sigs[tpl_id] # tpl_id的所有函数
    for func_hash, func_info in tpl_sig_s.items():
        origin_time = func_info[1] #
        if func_hash not in func_origin.keys():
            continue
        tpl_id_x, origin_time_x = func_origin[func_hash]  # tpl_id_x func_hash对应的origin tpl
                                                          # origin_time_x func在origin tpl的出生时间
        if tpl_id_x == tpl_id:  # origin tpl就是tpl_id自己 func不是重用函数
            continue
        if origin_time_x == 0 or origin_time_x < origin_time: # centris方法
            tpl_intersection[tpl_id_x].add(func_hash) # tpl_intersection[tpl_id_x]表示tpl_id_x被重用的函数
...
def main():
    ...
    with tqdm(total=tpl_num) as pbar:
        for res in pool.imap_unordered(resolve_source_relation, tpl_list):
            tpl_intersection_all.update(res)
                                            # tpl_intersection_all={tpl_id1:tpl_intersection1,tpl_id2:tpl_intersection2} tpl_id重用起源于tpl_id_x的func
                                            # tpl_intersection={tpl_id_x1:(func_hash1,func_hash2),tpl_id_x2:(func_hash3,func_hash4)}
            pbar.update()
    pool.close()
    pool.join()
    ...
    for tpl_id_s, tpl_intersection in tpl_intersection_all.items():
        for reuse_list in tpl_intersection.values(): # 跳过tpl_id_x
            tpl_reuse_set[tpl_id_s] |= set(reuse_list) #tpl_reuse_set[tpl_id_s] 表示tpl_id_s重用的函数
                                                       # {tpl_id:{tpl_id_x:func_hash}} ->{tpl_id:func_hash}

    recall_relation = set()
    tpl_reuse = defaultdict(list)
    for tpl_id_s, tpl_intersection in tpl_intersection_all.items():
        # 添加tpl_id_s reuse tpl_id_x边
        for tpl_id_x, reuse_list in tpl_intersection.items():
            # exclude the reused count
            # 去除
            func_num_x = len(tpl_sigs[tpl_id_x]) # tpl_id_x的所有函数个数
            reused_num_x = len(tpl_reuse_set[tpl_id_x])  # tpl_id_x重用的函数 /所有origin tpl是tpl_id_x的函数数量？
            tpl_len_x = len(tpl_sigs[tpl_id_x]) - len(tpl_reuse_set[tpl_id_x])
            if tpl_len_x < 1:
                continue
            if len(reuse_list) / func_num_x >= config.THRESHOLD * func_num_x / reused_num_x:# fixme
            # x被重用的函数个数/x所有函数个数 >= config.THRESHOLD * x所有函数/x重用的函数
